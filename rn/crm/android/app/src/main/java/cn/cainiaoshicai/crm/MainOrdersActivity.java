...
public class MainOrdersActivity extends AbstractActionBarActivity {
    ...
    public static Intent newIntent() {
        return new Intent(GlobalCtx.app(), MainOrdersActivity.class);
    }
...
//        ordersViewPager.setOffscreenPageLimit(adapter.getCount());
    ...
    private void resetPrinterStatusBar() {
        ...
        searchBtn.setOnClickListener(v -> GlobalCtx.app().toSearchActivity(MainOrdersActivity.this, ""));
        ...
        printerStatus.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                MainOrdersActivity.this.startActivity(i);
            }
        });
...
GlobalCtx.app().toTaskListActivity(MainActivity.this);
    ...
    private void initSignButton(final GlobalCtx app, final TextView signingText) {
        ...
        signingText.setOnClickListener(v -> {
            ...
            if (vendorId != STORE_VENDOR_CN) {
                GlobalCtx.app().toSetStoreStatusView(MainOrdersActivity.this);
            } else {
                if (signInStatus == Cts.SIGN_ACTION_IN) {
                    final ProgressFragment pf = ProgressFragment.newInstance(R.string.signing);
                    Utility.forceShowDialog(MainOrdersActivity.this, pf);
                        ...
                        protected Void doInBackground(Void... params) {
                            ...
                            try {
                                ...
                                if (msg != null && msg.isOk()) {
                                    MainOrdersActivity.this.runOnUiThread(new Runnable() {
                                        @Override
                                        public void run() {
                                            AlertUtil.showAlert(MainOrdersActivity.this, R.string.working_status,
                            ...
                            if (!TextUtils.isEmpty(err)) {
                                AlertUtil.errorOnActivity(MainOrdersActivity.this, "发生错误" + err);
                            }
                            return null;
                        }
                    }.executeOnNormal();
                    ...
                    Utility.showStoreSelector(MainOrdersActivity.this, "选择工作门店", "签到", "暂不签到", defStoreId,
                                        ...
                                        protected void onPostExecute(Void aVoid) {
                                            ...
                                            if (resultBean != null && resultBean.isOk()) {
                                                ...
                                                MainOrdersActivity.this.runOnUiThread(new Runnable() {
                                                    @Override
                                                    public void run() {
                                                        AlertUtil.showAlert(MainOrdersActivity.this, "门店提醒", okTips);
                                                        updateSignInStatus(obj, signingText);
                                                    }
                                                });
                                            } else {
                                                Utility.toast("签到失败:" + resultBean.getDesc(), MainOrdersActivity.this);
                                                MainOrdersActivity.this.runOnUiThread(new Runnable() {
                                                    @Override
                                                    public void run() {
    ...
    public void periodEnabledLocation() {
        if (locationHelper == null) {
            locationHelper = new LocationHelper(MainOrdersActivity.this);
        }
        ...
        if (store_id > 0) {
            ...
            shipStatusText.setOnClickListener(new View.OnClickListener() {
                ...
                void dlg(final long storeId) {
                    ...
                    AlertUtil.showAlert(MainOrdersActivity.this, R.string.ship_accept_status, msg,
                            "刷新状态", (dialog, which) -> app.dao.shippingAcceptStatus(storeId).enqueue(new Callback<ResultBean<ShipAcceptStatus>>() {
                                ...
                                public void onResponse(Call<ResultBean<ShipAcceptStatus>> call,
                                    ...
                                    if (body != null && body.isOk()) {
                                        ...
                                        AlertUtil.error(MainOrdersActivity.this, "刷新失败，请稍后重试：" + body.getDesc());
                                    }
                                }

                                @Override
                                public void onFailure(Call<ResultBean<ShipAcceptStatus>> call, Throwable t) {
                                    AlertUtil.error(MainOrdersActivity.this, "系统错误，稍后重试");
                                }
                            })
                            ,
                            "关闭", null,
    ...
    private void showChoosePrinterDialog(final long storeId) {
        ...
        singleChoiceDialogFragment.show("请选择接单打印机", items, checkedIdx, (dialog, which) -> {
        ...
        }, (dialog, which) -> singleChoiceDialogFragment.dismiss(), getFragmentManager());
    }
        ...
        resultBeanCall.enqueue(new Callback<ResultBean<ShipAcceptStatus>>() {
            ...
            public void onResponse(Call<ResultBean<ShipAcceptStatus>> call,
                ...
                if (body != null && body.isOk()) {
                    ...
                    app.getAccountBean().setShipAcceptStatus(body.getObj());
                    initShipAccept(app);
                    Utility.toast("操作成功", MainOrdersActivity.this);
                } else {
                    AlertUtil.error(MainOrdersActivity.this, "操作失败，请稍后重试：" + Objects.requireNonNull(body).getDesc());
                }
            }
    ...
    private void requestUpdateBadges() {
        GlobalCtx.app().getTaskCount(MainOrdersActivity.this, new GlobalCtx.TaskCountUpdated() {
            @Override
    ...
    public HashMap<String, String> extraEnvInfo() {
        ...
        if (location != null) {
            ...
            AlertUtil.showAlert(MainOrdersActivity.this, "签到失败", "无法获得当前的地理位置，开启位置权限后重试！");
        }
...
//                return true;
            ...
            case R.id.menu_store_maint:
                return GlobalCtx.app().toGoodsMgrRN(MainOrdersActivity.this);
...
// so we can call this asynchronously
        ...
        if (requestCode == REQUEST_DAY) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && BuildConfig.DEBUG) {
                if (!Settings.canDrawOverlays(this)) {
                    ...
                    AlertUtil.error(MainOrdersActivity.this, "授权ALERT_WINDOW限失败");
                }
            }
        }
    ...
    protected void onDestroy() {
        super.onDestroy();
        EventBus.getDefault().unregister(MainOrdersActivity.this);
    }
    ...
    private class SignOffOnClickListener implements DialogInterface.OnClickListener {
        ...
        public void onClick(DialogInterface dialog, int which) {
            AlertDialog.Builder adb = new AlertDialog.Builder(MainOrdersActivity.this);
                    ...
                    .setPositiveButton("现在下班", new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            for (boolean aChecked : checked) {
                                if (!aChecked) {
                                    AlertUtil.showAlert(MainOrdersActivity.this, "", "店内各项工作完成方可打卡下班");
                                    return;
                                }
                            }
                                ...
                                protected void onPostExecute(Void aVoid) {
                                    ...
                                    if (resultBean != null && resultBean.isOk()) {
                                        MainOrdersActivity.this.runOnUiThread(new Runnable() {
                                            @Override
                                            public void run() {
                                                AlertUtil.showAlert(MainOrdersActivity.this, "下班提示", resultBean.getDesc());
                                                updateSignInStatus(resultBean.getObj(), signingText);
                                            }
                                        });
                                    } else {
                                        Utility.toast("打卡失败:" + resultBean.getDesc(), MainOrdersActivity.this);
                                        MainOrdersActivity.this.runOnUiThread(new Runnable() {
                                            @Override
                                            public void run() {
                                                updateSignInStatus(resultBean.getObj(), signingText);
...
//            GeneralWebViewActivity.gotoWeb(MainActivity.this, url);
            ...
            params.put("url", url);
            GlobalCtx.app().toRNWebView(MainOrdersActivity.this, params);
        }
    }
}
