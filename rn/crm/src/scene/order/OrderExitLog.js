              ...
              <View>
                <For each="log" index="logIdx" of={item.exitLog}>
                  <View key={log.id} style={{padding: 10, borderBottomWidth: 1, borderBottomColor: '#f8f8f8'}}>
                    {log.pack_time ? (<Text style={{color: colors.color333}}>打包时间：{log.pack_time} </Text>) : null}
                    {log.packUsername ? (<Text style={{color: colors.color333}}>打包人：{log.packUsername} </Text>) : null}
                    {log.weight ? (<Text style={{color: colors.color333}}>重量：{log.weight} </Text>) : null}
                    {log.num ? (<Text style={{color: colors.color333}}>数量：{log.num} </Text>) : null}
                  </View>
                </For>
              </View>
            </Accordion.Panel>
