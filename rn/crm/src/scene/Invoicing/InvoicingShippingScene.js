      ...
      reqList = lockedList.map(function (item, idx) {
        return <Cell access customStyle={Styles.in_h_cell} onPress={() => {
          ...
          <CellHeader style={{justifyContent: 'center', height: pxToDp(180)}}>
            ...
            {!!item['remark'] ? <View style={{flexDirection: 'row', flexWrap:'nowrap'}}><Text
                style={[font.font24, font.fontBlack, {marginTop: pxToDp(10)}]}>备注:</Text><Text
                style={[font.font24, font.fontRed, {marginTop: pxToDp(11), flexWrap: 'wrap', width: pxToDp(450)}]}> {item['remark'].replace(/\s/g, ",")}</Text></View> :
