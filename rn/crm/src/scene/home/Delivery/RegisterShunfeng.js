...
class RegisterShunfeng extends PureComponent {
    ...
    startUploadImg = (imgPath, imgName) => {
        showModal("图片上传中...")
        const newImageKey = tool.imageKey(imgName)
        ...
        QNEngine.eventEmitter({
            ...
            onComplete: (data) => {
                HttpUtils.get.bind(this.props)('/qiniu/getOuterDomain', {bucket: 'goods-image'}).then(res => {
                    showSuccess('上传成功')
                    const {newImageKey} = this.state
                    const imageUrl = res + newImageKey
    ...
    menus = [{label: '拍照', onPress: this.pickCameraImg}, {
        label: '从相册选择',
