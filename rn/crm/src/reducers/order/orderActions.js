...
export function cancelReasonsList(ship_id, token, callback) {
  return dispatch => {
    const url = `api/third_ship_cancel_reasons/${ship_id}.json?access_token=${token}`;
    FetchEx.timeout(AppConfig.FetchTimeout, FetchEx.get(url))
        .then(resp => resp.json())
        .then(resp => {
          callback(resp);
...
export function cancelShip(ship_id, reason_id, token, callback) {
  const url = `api/third_ship_cancel/${ship_id}/${reason_id}.json?access_token=${token}`;
  return jsonReqThenInvalidate(url, ship_id, callback);
}
