...
export function cancelReasonsList(order_id,token,callback) {
  return dispatch => {
    const url = `api/order_dada_cancel_reasons/${order_id}.json?access_token=${token}`;
    FetchEx.timeout(AppConfig.FetchTimeout, FetchEx.get(url))
        .then(resp => resp.json())
        .then(resp => {
          callback(resp);
  ...
  export function cancelShip(order_id,reason_id,data,token,callback) {
    const url = `api/order_dada_cancel/${order_id}/${reason}.json?access_token=${token}`;
    console.log(url,id, reason_id, data, token);
    return false;
